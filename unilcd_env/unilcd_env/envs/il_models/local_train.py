...
from local_dataloader import get_dataloader
...
def train(data_folder, save_path):
    ...
    infer_action = LocalModel()
    infer_action.to(device)
    infer_action.requires_grad = True
    ...
    for epoch in range(nr_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            batch_out = infer_action(batch_in, batch_gt2)
    ...
    plt.title('Loss Plot for Local Only Model')
    plt.plot(loss_values)#plotting loss values
    plt.savefig('Learning_Curve_local.jpg')#saving the plot
